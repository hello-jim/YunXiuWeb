...
namespace BrnMall.Data
{
    ...
    public partial class OrderAfterServices
    {
        ...
        {
            BrnMall.Core.BMAData.RDBS.ApplyOrderAfterService(state, uid, oid, recordId, pid, cateId, brandId, storeId, pname, pshowImg, count, money, type, applyReason, applyTime);
        }
        ...
        {
            BrnMall.Core.BMAData.RDBS.CheckOrderAfterService(asId, state, checkResult, checkTime);
        }
        ...
        {
            BrnMall.Core.BMAData.RDBS.SendOrderAfterService(asId, state, shipCoId, shipCoName, shipSN, regionId, consignee, mobile, phone, email, zipCode, address, sendTime);
        }
        ...
        {
            BrnMall.Core.BMAData.RDBS.ReceiveOrderAfterService(asId, state, receiveTime);
        }
        ...
        {
            BrnMall.Core.BMAData.RDBS.BackOrderAfterService(asId, state, shipCoId, shipCoName, shipSN, backTime);
        }
        ...
        {
            BrnMall.Core.BMAData.RDBS.CompleteOrderAfterService(asId, state);
        }
        ...
        {
            List<OrderAfterServiceInfo> orderAfterServiceList = new List<OrderAfterServiceInfo>();
            IDataReader reader = BrnMall.Core.BMAData.RDBS.GetOrderProductAfterServiceList(recordId);
            while (reader.Read())
            {
                OrderAfterServiceInfo orderAfterServiceInfo = BuildOrderAfterServiceFromReader(reader);
                orderAfterServiceList.Add(orderAfterServiceInfo);
        ...
        {
            return BrnMall.Core.BMAData.RDBS.GetOrderProductAfterServiceList(pageSize, pageNumber, condition);
        }
        ...
        {
            return BrnMall.Core.BMAData.RDBS.GetOrderProductAfterServiceCount(condition);
        }
        ...
        {
            return BrnMall.Core.BMAData.RDBS.GetOrderProductAfterServiceListCondition(storeId, uid, oid, state, type, applyStartTime, applyEndTime);
        }
        ...
        public static OrderAfterServiceInfo GetOrderAfterServiceByASId(int asId)
        {
            ...
            IDataReader reader = BrnMall.Core.BMAData.RDBS.GetOrderAfterServiceByASId(asId);
            if (reader.Read())
            {
