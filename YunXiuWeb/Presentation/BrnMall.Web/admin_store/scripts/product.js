...
function selectCommonAttrValue(obj) {
    ...
    if (productPageType == 1) {
        ...
        if (attrValueObj.prop("checked") == false) {
            changeProductAttributeUrl = "/storeadmin/product/delproductattribute?pid=" + pid + "&attrId=" + attrValueObj.parent().find("input[name=AttrIdList]").val() + "&t=" + new Date();
        }
        else {
            if (isInput == "0") {
                changeProductAttributeUrl = "/storeadmin/product/updateproductattribute?type=0" + "&pid=" + pid + "&attrId=" + attrValueObj.parent().find("input[name=AttrIdList]").val() + "&attrValueId=" + attrValueObj.val() + "&t=" + new Date();
            }
        }
...
$(function () {
    ...
    if (productPageType != 2 && cateId > 0) {
        ...
        if (productPageType == 0 || productPageType == 1) {
            ...
            $.get("/storeadmin/category/aandvjsonlist?cateId=" + cateId + "&time=" + new Date(), function (data) {
                ...
                $(".inputattrvalue").on("blur", function () {
                    ...
                    if (inputAttrValueObj.val() != oldInputAttrValue) {
                        $.jBox.tip("正在更新...", 'loading');
                        $.get("/storeadmin/product/updateproductattribute?type=1" + "&pid=" + pid + "&attrId=" + inputAttrValueObj.parent().find("input[name=AttrIdList]").val() + "&attrValueId=" + inputAttrValueObj.attr("attrValueId") + "&inputValue=" + encodeURIComponent(inputAttrValueObj.val()) + "&t=" + new Date(), function (data, textStatus) {
                            if (data != "0") {
                                $.jBox.tip('更新成功！', 'success');
                            } else {
        ...
        $(".shoppriceinput").blur(function () {
            ...
            shopPriceInputObj.prop("class", "unselectedsortinput");
        });
