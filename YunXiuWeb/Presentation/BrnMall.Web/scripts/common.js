...
//店铺搜索
function storeSearch(storeId, word, storeCid, startPrice, endPrice) {
    if (storeId < 1) {
        alert("请先选择店铺");
    }
    else if ((word == undefined || word == null || word.length < 1) && storeCid < 1 && (startPrice == undefined || startPrice == null || startPrice.length < 1) && (endPrice == undefined || endPrice == null || endPrice.length < 1)) {
        alert("请输入搜索条件");
    }
    else {
        window.location.href = "/store/search?storeId=" + storeId + "&word=" + encodeURIComponent(word) + "&storeCid=" + storeCid + "&startPrice=" + startPrice + "&endPrice=" + endPrice;
    }
}
...
function setBatchSelectCartItemCheckbox() {
    ...
    $("#cartBody input[type=checkbox][name=cartItemCheckbox]:not(:checked)").each(function () {
        $("#storeCartCheckbox" + $(this).attr("storeId")).prop("checked", false);
        flag = false;
    })
...
function cancelOrSelectStoreCart(obj) {
    ...
    $("#cartBody input[type=checkbox][name=cartItemCheckbox][storeId=" + storeId + "]").each(function () {
