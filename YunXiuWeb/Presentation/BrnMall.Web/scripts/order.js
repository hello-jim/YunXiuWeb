...
function submitOrder() {
    ...
    if (!verifySubmitOrder(saId, buyerRemark)) {
        return;
    }

    $.post("/order/submitorder",
            { 'selectedCartItemKeyList': selectedCartItemKeyList, 'saId': saId, 'payMode': payMode, 'payCreditCount': payCreditCount, 'couponIdList': couponIdList, 'couponSNList': couponSN, 'fullCut': allFullCut, 'bestTime': bestTime, 'buyerRemark': buyerRemark, 'verifyCode': verifyCode },
            submitOrderResponse)
}

//验证提交订单
function verifySubmitOrder(saId, buyerRemark) {
    if (saId < 1) {
        alert("请填写收货人信息");
        return false;
    }
